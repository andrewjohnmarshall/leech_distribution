...
hist(d.p$H.picta, 
     col = "orange",
     main = expression(italic("H. picta")),
     ylab = "Frequency",
     xlab = " ",
     las = 1,
     xlim = c(0,20),
...
hist(d.z$H.zeylanica, 
     col = "brown", 
     main  = expression(italic("H. zeylanica")),
     xlab = "Counts", 
     ylab = "Frequency",
     las = 1,
     xlim = c(0,20),
...
barplot(obs.count.table.p.1$`Average counts`, 
        ...
        horiz = TRUE,
        las = 1,
        xlab = substitute(paste(italic('H. picta'))), 
        ylab = "",
        col = "orange",
        cex.lab = 0.9, 
        cex.axis = 0.7,
...
barplot(obs.count.table.z.1$`Average counts`, 
        ...
        horiz = TRUE,
        las = 1, 
        xlab = substitute(paste(italic('H. zeylanica'))), 
        ylab = "",
        col = "chocolate4",
        cex.lab = 0.9, 
        cex.axis = 0.7,
...
plot1 <- histogram(H.picta~factor(FT)|factor(Distance),
          data=d.p,
          xlab = "Forest type",
          ylab = substitute (paste("% of total counts -", italic(" H. picta"))),
          col = c("orange"),
          layout = c(1, 11))
...
plot2 <- histogram(H.zeylanica~factor(FT)|factor(Distance),
          data=d.p,
          xlab = "Forest type",
          ylab = substitute (paste("% of total counts -", italic(" H. zeylanica"))),
          col = c("chocolate4"),
          layout = c(1, 11))
...
legend("topright",
       legend =c(expression(italic('H. picta')), expression(italic('H. zeylanica'))), 
...
# cut-off point - pair-wise correlation of 0.5 and higher. Thus, won't use forest type and altitude in the same model to avoid issues with convergence.
...
# reorder columns
n <- c(14, 12, 13, 10, 11, 8, 9, 4, 6, 7, 5, 2, 3, 1)
...
mtext("(a)",line = 0,
       side = 3, at = 0,
       cex = 0.7, adj = 0)
mtext("      H. picta",line = 0,
       side = 3, at = 0,
       cex = 0.7, adj = 0, font = 3)
...
# reorder columns
n <- c(12, 13, 10, 11, 8, 9, 2, 3, 4, 6, 7, 5, 1)
...
axis(side = 2, 
      ...
      labels =  c("Observer Jack", "Soil is wet", "Soil is moist", "Raining now", "Rained last night", "Rained yest.", 
                  "Rained day before yest.", "Montane", "Upland granite", "Lowland granite", "Lowland sandstone", 
                  "Alluvial bench", "Freshwater swamp"),
      cex.axis = 0.5,
      las = 1, 
      mgp = c(0, 0.35, 0), #middle number is the only one that's relevant here: the spacing between ticks and tick labels
      tcl = -0.2) 
...
xyplot(H.zeylanica ~ H.picta | FT, data = d.p, 
       group = FT,
       xlab = expression (paste("Counts of", italic(" H. picta"))), 
       ylab = expression (paste("Counts of", italic(" H. zeylanica"))), 
       cex = 0.5, 
       pch = 16,
       col = c("#0068374d"),
       ...
       as.table = TRUE, #if I don't use this, the order starts from peat swamp i.e. from low altitude to high altitude
       index.cond = list(c(6,7,4,5,2,3,1)), #reorder panels - https://stackoverflow.com/questions/6358865/how-to-change-the-order-of-the-panels-in-simple-lattice-graphs
